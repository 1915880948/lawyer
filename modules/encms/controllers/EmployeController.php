...
class EmployeController extends BaseController {
    public function actionList() {
        if( Yii::$app->request->isAjax ){
            ...
            $query = Employe::find()->andWhere(['status' => 1]);
    ...
    public function actionEdit() {
        if (Yii::$app->request->isAjax) {
            ...
            $data = [];
            $attributes = AttributeHelper::filter((new Employe())->attributes());
            foreach ($attributes as $list){
                $data[$list] = isset($post[$list]) ? $post[$list] : '';
            }
            if( $post['id'] ){
                $model = Employe::findOne(['id'=>(int)$post['id']]);
            }else{
                $model = new Employe();
                $model->status = 1;
    ...
    public function actionDetail() {
        ...
        $data = Employe::find()->andWhere(['id' => $id])->one();
    ...
    public function actionOperate() {
        ...
        $model = Employe::findOne(['id'=>(int)$id]);
